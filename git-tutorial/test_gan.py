...
#                                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
#                            ]))
dataset = Dataset(dataroot, dataroot2, transform=transforms.Compose([
                               transforms.Resize(image_size),
                               transforms.CenterCrop(image_size),
                               transforms.ToTensor(),
                               transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
...
class Generator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.input = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=1, padding=1, bias=False).cuda()
        self.inputRB = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, bias=False).cuda()
        self.conv1 = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, bias=False).cuda()
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1, bias=False).cuda()
        self.output = nn.Conv2d(in_channels=16, out_channels=3, kernel_size=3, stride=1, padding=1, bias=False).cuda()
    ...
    def RBlock(self, x):
        inputs = self.inputRB(self.relu(x))
        out = inputs
        for _ in range(3):
            out = self.conv2(self.relu(self.conv1(self.relu(out))))
            out = torch.add(out, inputs).cuda()
        return out
        
    def forward(self, input):
        residual = input
        x = self.input(self.relu(input))
        for _ in range(2):
            x = self.RBlock(x).cuda()
        x = self.output(self.relu(x))
        out = torch.add(x, residual).cuda()
        return out
...
for epoch in range(num_epochs):
    ...
    for i, data, data2 in enumerate(dataloader, 0):
